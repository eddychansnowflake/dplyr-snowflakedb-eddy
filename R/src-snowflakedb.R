...
db_insert_into.SnowflakeDBConnection <- function(con, table, values, ...) {
  ...
  sql <- dplyr::build_sql("COPY INTO ", ident(table), " FILE_FORMAT = (FIELD_DELIMITER = '\\t' SKIP_HEADER = 1 NULL_IF = 'NA' ESCAPE_UNENCLOSED_FIELD = '\\' FIELD_OPTIONALLY_ENCLOSED_BY = '\042')")
  message(sql)
  rs <- dbGetQuery(con, sql)
